...
class DetailState {
  ...
  DetailState copyWith({
    ...
    return DetailState(
      ...
      forumAvatar: forumAvatar ?? this.forumAvatar,
...
class DetailController extends StateNotifier<DetailState> {
  ...
  DetailController(this._repository, this.threadId, [this._ref])
  ...
  Future<void> refresh() async {
    ...
    try {
      ...
      final serverCreateTime =
          int.tryParse(threadMap['create_time']?.toString() ?? '0') ?? 0;
      final authorMap = threadMap['author'] is Map
          ? threadMap['author'] as Map
          : null;
      final serverIp =
          threadMap['ip_address']?.toString() ??
      ...
      final serverFid =
          forumMap['id']?.toString() ??
          threadMap['fid']?.toString() ??
          threadMap['forum_id']?.toString() ??
          '';
      final serverFname =
          forumMap['name']?.toString() ??
          threadMap['fname']?.toString() ??
          threadMap['forum_name']?.toString() ??
          '';
      final serverForumAvatar =
          forumMap['avatar']?.toString() ??
          forumMap['forum_avatar']?.toString() ??
          threadMap['forum_avatar']?.toString() ??
          threadMap['avatar']?.toString() ??
          '';

      final agreeMap = threadMap['agree'] is Map
          ? (threadMap['agree'] as Map)
          : null;
      final serverAgreeNum = int.tryParse(
        agreeMap?['agree_num']?.toString() ??
            agreeMap?['diff_agree_num']?.toString() ??
            threadMap['agree_num']?.toString() ??
            '0',
      );
      final serverHasAgreed =
      ...
      final serverFirstPostId =
          threadMap['first_post_id']?.toString() ??
      ...
      final bool finalAgreed = globalRecord != null
          ? globalRecord.isAgreed
          : serverHasAgreed;
      final int finalAgreeNum = globalRecord != null
          ? globalRecord.agreeNum
          : (serverAgreeNum ?? 0);

      // 同步到全局 Provider
      _ref
          ?.read(threadAgreeProvider.notifier)
          .setAgree(threadId, finalAgreed, finalAgreeNum);
      ...
      state = state.copyWith(
        ...
        forumAvatar: serverForumAvatar.isNotEmpty
            ? serverForumAvatar
            : state.forumAvatar,
        tbs:
            (res['anti'] is Map &&
                res['anti']['tbs'] != null &&
                res['anti']['tbs'].toString().isNotEmpty)
      ...
      state = state.copyWith(isRefreshing: false, errorMessage: '加载失败，请重试');
    }
  }
  ...
  Future<void> toggleThreadAgree({
    ...
    final newAgreeNum = (currentAgreeNum + (newAgreed ? 1 : -1)).clamp(
      0,
      999999,
    );
    ...
    _ref
        ?.read(threadAgreeProvider.notifier)
        .setAgree(threadId, newAgreed, newAgreeNum);

    final targetPostId = (firstPostId != null && firstPostId.isNotEmpty)
        ? firstPostId
        : (state.firstPostId.isNotEmpty
              ? state.firstPostId
              : (state.floors.isNotEmpty && state.floors.first.floor == 1
                    ? state.floors.first.id
                    : '0'));
    ...
    final tbs =
        await _ref?.read(authStateProvider.notifier).getValidTbs() ??
        account.tbs;
    ...
    if (!result.success &&
        (result.errorMsg.contains('TBS') || result.errorMsg.contains('tbs'))) {
      final freshTbs =
          await _ref
    ...
    if (!result.success) {
      ...
      _ref
          ?.read(threadAgreeProvider.notifier)
          .setAgree(threadId, currentAgreed, currentAgreeNum);
      if (updatedFloors.isNotEmpty && updatedFloors.first.floor == 1) {
        updatedFloors[0] = updatedFloors.first.copyWith(
      ...
      AppToast.showToast(
        result.errorMsg.isNotEmpty ? result.errorMsg : '点赞失败，请重试',
      );
    } else {
      // 同步到 feed 控制器
      try {
        _ref
            ...
            .syncThreadAgree(threadId, newAgreed, newAgreeNum);
        _ref
  ...
  Future<void> toggleFloorAgree(String floorId) async {
    ...
    if (oldFloor.floor == 1) {
      final globalRecord = _ref?.read(threadAgreeProvider)[threadId];
      final curAgreed =
          globalRecord?.isAgreed ?? state.isAgreed ?? oldFloor.isAgreed;
      final curAgreeNum =
          globalRecord?.agreeNum ?? state.agreeNum ?? oldFloor.agreeNum;
    ...
    final newAgreeNum = (oldFloor.agreeNum + (newAgreed ? 1 : -1)).clamp(
      0,
      999999,
    );
  ...
  void setForumInfo(String fid, String fname, [String forumAvatar = '']) {
    if (fid.isEmpty && fname.isEmpty && forumAvatar.isEmpty) return;
    state = state.copyWith(
      forumId: fid.isNotEmpty ? fid : state.forumId,
      forumName: fname.isNotEmpty ? fname : state.forumName,
      forumAvatar: forumAvatar.isNotEmpty ? forumAvatar : state.forumAvatar,
    );
  }
  ...
  Future<bool> sendReply({
    ...
    final fname =
        (state.forumName.isNotEmpty ? state.forumName : fallbackFname) ?? '';
    ...
    try {
      ...
      if (!result.success &&
        ...
        if (freshTbs.isNotEmpty && freshTbs != tbs) {
          result = await _repository.addReply(
            ...
            replyUserName: replyUserName,
            replyUserPortrait: replyUserPortrait,
            nameShow: account.nameShow.isNotEmpty
                ? account.nameShow
                : account.name,
          );
        }
      ...
      if (result.success) {
        ...
        AppToast.showToast(
          result.errorMsg.isNotEmpty ? result.errorMsg : '回复失败，请重试',
        );
        return false;
      }
    } catch (e) {
  ...
  Future<bool> deleteFloor(
    ...
    if (result.success) {
      ...
      final updatedList = List<TiebaFloorModel>.from(state.floors)
        ..removeAt(floorIndex);
      state = state.copyWith(floors: updatedList);
      return true;
    } else {
      AppToast.showToast(
        result.errorMsg.isNotEmpty ? result.errorMsg : '删除失败，请重试',
      );
      return false;
    }
  }
...
final detailControllerFamily =
    StateNotifierProvider.family<DetailController, DetailState, String>((
      ref,
      threadId,
    ) {
