...
class ForumController extends StateNotifier<ForumState> {
  ...
  Future<void> refresh() async {
    ...
    try {
      ...
      if (forum != null) {
        ...
        final matched = myForums.where(
          (f) =>
              f.name == forumName || (forum!.id.isNotEmpty && f.id == forum.id),
        );
      ...
      state = state.copyWith(
        forum: forum,
        pinnedThreads:
            (res['pinned_threads'] as List?)?.cast<TiebaThreadModel>() ?? [],
  ...
  Future<void> loadMore() async {
    ...
    try {
      ...
      state = state.copyWith(
        threads: [
          ...state.threads,
          ...(res['threads'] as List?)?.cast<TiebaThreadModel>() ?? [],
        ],
        page: nextPage,
        hasMore: res['has_more'] as bool? ?? false,
        isLoading: false,
  ...
  Future<bool> signIn() async {
    ...
    try {
      ...
      if (success) {
        ...
        await _ref.read(myForumsControllerProvider.notifier).refresh();
        return true;
      }
    } catch (_) {}
  ...
  Future<({bool success, String errorMsg})> unfollowForum() async {
    ...
    if (currentForum == null) {
      final myForums = _ref.read(myForumsControllerProvider).forums;
      final matched = myForums.where((f) => f.name == forumName);
      final fid = matched.isNotEmpty
          ? matched.first.id
          : (_repository.getCachedForumId(forumName) ?? '');
      currentForum = ForumDetailModel(id: fid, name: forumName);
    }
    if (state.isFollowing) return (success: false, errorMsg: '正在处理中');
    ...
    try {
      ...
      if (result.success) {
        state = state.copyWith(
          isFollowing: false,
          forum: (state.forum ?? currentForum).copyWith(
            isLiked: false,
            isSigned: false,
          ),
        );
...
final forumControllerFamily =
    StateNotifierProvider.family<ForumController, ForumState, String>((
      ref,
      forumName,
    ) {
      ...
      return ForumController(
        ...
        ref,
        forumName,
        initialSortType: habitState.forumDefaultSort,
      );
    });
