...
class TiebaAuthorModel {
  ...
  String get displayName =>
      nameShow.isNotEmpty ? nameShow : (name.isNotEmpty ? name : '贴吧吧友');
  ...
  factory TiebaAuthorModel.fromJson(
    ...
    final rawShowInfo =
        json?['user_show_info'] ?? userDetail?['user_show_info'];
    final showInfo = rawShowInfo is Map
        ? (rawShowInfo is Map<String, dynamic>
              ? rawShowInfo
              : Map<String, dynamic>.from(rawShowInfo))
        : null;
    if (showInfo != null) {
      final feedHead = showInfo['feed_head'] is Map
          ? (showInfo['feed_head'] as Map)
          : null;
      if (feedHead != null) {
    ...
    final rawIconList =
        json?['show_icon_list'] ?? userDetail?['show_icon_list'];
    ...
    if (iconList != null) {
      for (var icon in iconList) {
        if (icon is Map &&
          ...
          if (t.isNotEmpty &&
              (nameShow.isEmpty || nameShow.startsWith('贴吧用户_'))) {
            nameShow = t;
          }
        }
      }
...
class TiebaVideoInfoModel {
  ...
  factory TiebaVideoInfoModel.fromJson(Map<String, dynamic> json) {
    ...
    final w =
        int.tryParse(
          json['video_width']?.toString() ?? json['width']?.toString() ?? '0',
        ) ??
        0;
    final h =
        int.tryParse(
          json['video_height']?.toString() ?? json['height']?.toString() ?? '0',
        ) ??
        0;
    return TiebaVideoInfoModel(
      videoUrl: url,
      coverUrl: cover,
      duration: dur,
      width: w,
...
class TiebaThreadModel {
  ...
  factory TiebaThreadModel.fromJson(
    ...
    final id =
        ...
        '';
    final title =
        json['title']?.toString() ?? json['thread_title']?.toString() ?? '';
    final fname =
        json['fname']?.toString() ?? json['forum_name']?.toString() ?? '';
    ...
    if (json['video_info'] is Map) {
      final vModel = TiebaVideoInfoModel.fromJson(
        Map<String, dynamic>.from(json['video_info'] as Map),
      );
      if (vModel.videoUrl.isNotEmpty &&
          !vModel.videoUrl.toLowerCase().endsWith('.jpg')) {
        videoInfo = vModel;
      }
    ...
    return TiebaThreadModel(
      ...
      fid: fid,
      forumAvatar:
          json['forum_avatar']?.toString() ?? json['avatar']?.toString() ?? '',
      contentSnippet: snippet,
      replyNum:
          int.tryParse(
            json['reply_num']?.toString() ?? json['count']?.toString() ?? '0',
          ) ??
          0,
      agreeNum: agreeNum,
      isAgreed: hasAgreed,
      ...
      createTime:
          int.tryParse(
            json['create_time']?.toString() ??
                json['time']?.toString() ??
                json['last_time']?.toString() ??
                '0',
          ) ??
          0,
      videoInfo: videoInfo,
